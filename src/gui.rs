...
impl EframeWrapper {
    ...
    fn render_app_list(&mut self, ui: &mut eframe::egui::Ui, ctx: &eframe::egui::Context) {
        ...
        let filtered = if query.trim().is_empty() {
            if cfg.enable_recent_apps { self.app.get_search_results().into_iter().take(cfg.max_search_results).collect() } else { Vec::new() }
        } else { self.app.get_search_results().into_iter().take(cfg.max_search_results).collect() };
        ui.vertical(|ui| {
...
impl eframe::App for EframeWrapper {
    fn update(&mut self, ctx: &eframe::egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.app.should_quit() { process::exit(0); }
    }
}

pub fn load_theme() -> Result<Theme, Box<dyn Error>> { Theme::load_or_create() }
